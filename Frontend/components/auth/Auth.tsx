                ...
                <Card.Section>
                    <Paper withBorder p={"xl"} sx={(theme) => ({
                        ...
                        <Box mb={'md'}>
                            ...
                            <Button fullWidth variant={"outline"} mb={"sm"} size={'xs'}
                                    hidden={authState === AuthState.SIGN_UP || authState === AuthState.FORGOT_PASSWORD}
                                    onClick={() => setAuthState(AuthState.FORGOT_PASSWORD)}>
                                <a>Forgot password?</a>
                            </Button>

                            <Button fullWidth variant={'outline'} size={'xs'} mb={"sm"} hidden={authState === AuthState.SIGN_UP}
                                    onClick={() => setAuthState(AuthState.SIGN_UP)}>
                                <a>Need an account? SignUp here. </a>
                            </Button>

                            <Button fullWidth variant={'outline'} size={'xs'} mb={"sm"} hidden={authState === AuthState.LOGIN}
