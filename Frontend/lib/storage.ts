...
export async function removeProductPic(supabase: SupabaseClient, filenames: string[]) {
    const {data, error} = await supabase.storage
        .from('products')
        .remove([...filenames])
    return {data: data, error: error}
...
export function getImageUrl(supabase: SupabaseClient, filename: string) {
    if (filename.indexOf('http') === 0)
        return filename
    return supabase.storage.from("products")
        .getPublicUrl(filename, {download: false}).data.publicUrl
}
